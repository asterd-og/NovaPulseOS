...
override CPPFILES := $(shell find ./src/ -name "*.c")
override ASMFILES := $(shell find ./src/ -name "*.asm")
override OBJ := $(CPPFILES:.c=.o)
...
run:
	qemu-system-x86_64 -m 2G -smp 1 -s -cdrom $(ISONAME) -boot d -no-reboot -no-shutdown --serial stdio
...
kernel: $(OBJ)
	$(LD) $(shell find ./src/ -name "*.o") $(LDFLAGS) $(INTERNALLDFLAGS) -o $(KOUT)
...
$(ISONAME): limine kernel initrd
	...
	@mkdir -p iso_root
	@cp $(KOUT) \
		conf/limine.cfg conf/kfont.psf limine/limine.sys limine/limine-cd.bin limine/limine-cd-efi.bin iso_root/
